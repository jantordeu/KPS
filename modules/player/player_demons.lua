...
local function registerEventsIfNecessary()
    if not loaded then
        loaded = true
            ...
            local time, combatEvent, hidding, sourceGUID, sourceName, sourceFlags, sourceFlag2, destGUID, destName, destFlags, destFlag2, spellID, spellName, spellType, amount, overKill, school, resisted, blocked, absorbed, isCritical = CombatLogGetCurrentEventInfo()
            -- time out any demons
