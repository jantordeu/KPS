    ...
    return function ( )
        ...
        if spell ~= nil and target ~= nil and conditionFn() then
            if spell.id == nil then -- Cast Sequence!
                local castsequence = true
                for _,spells in ipairs(spell) do
                	if not spells().canBeCastAt(target) then castsequence = false end
                end
                if castsequence == true then
                    return spell, target
                end
    ...
    for _, spellTable in ipairs(hydraTable) do
        ...
        elseif type(spellTable[1]) == "table" and spellTable[1].name == nil then
            ...
            elseif spellTable[1][1].name ~= nil then
                ...
                table.insert(compiledTable, fnParseDefault(spellSequence, spellTable[2], spellTable[3], spellTable[4]))
            end
        else
