...
kps.rotations.register("WARRIOR","FURY",
{
    ...
    {spells.charge, 'kps.defensive and target.isAttackable and target.distanceMax  >= 10' },
    {spells.battleShout, 'not player.hasBuff(spells.battleShout)' },

    -- interrupts
    {spells.pummel, 'kps.interrupt and target.distanceMax  <= 10 and target.isInterruptable and target.castTimeLeft < 1' , "target" },
    {spells.pummel, 'kps.interrupt and focus.distanceMax  <= 10 and focus.isInterruptable and focus.castTimeLeft < 1' , "focus" },
    ...
    {spells.recklessness, 'kps.cooldowns and player.hasTalent(7,1) and player.rage <= 25 and target.isAttackable and target.distanceMax  <= 10' },
    {spells.recklessness, 'kps.cooldowns and not player.hasTalent(7,1) and player.rage >= 75 and target.isAttackable and target.distanceMax  <= 10' },
    ...
    {{"nested"}, 'spells.recklessness.cooldown >= 30 and target.distanceMax  <= 10 and target.isAttackable', {
    ...
    {spells.bloodthirst, 'not player.hasBuff(spells.enrage) and target.isAttackable and target.distanceMax  <= 10' , "target" },
    {spells.ragingBlow, 'spells.ragingBlow.charges == 2 and target.isAttackable and target.distanceMax  <= 10' , "target" , "ragingBlow_charges" },
    {spells.bloodthirst, 'target.isAttackable and target.distanceMax  <= 10' , "target" },
    {spells.ragingBlow, 'target.isAttackable and target.distanceMax  <= 10' , "target" , "ragingBlow_charges"},
    {spells.furiousSlash, 'player.hasTalent(3,3) and player.myBuffDuration(spells.furiousSlash) < 4 and target.isAttackable and target.distanceMax  <= 10' , "target" , "furiousSlash_buff"},
    {spells.furiousSlash, 'player.hasTalent(3,3) and player.buffStacks(spells.furiousSlash) < 3 and target.isAttackable and target.distanceMax  <= 10' , "target" , "furiousSlash_stacks"},
    {spells.whirlwind, 'target.isAttackable and target.distanceMax  <= 10' , "target" },
