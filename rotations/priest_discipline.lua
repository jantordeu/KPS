...
kps.runAtEnd(function()
   kps.gui.addCustomToggle("PRIEST","DISCIPLINE", "control", "Interface\\Icons\\spell_nature_polymorph", "control")
end)
...
--    {spells.fireBlood, 'player.isDispellable("Curse")' , "player" },
    ...
    {{"nested"}, 'player.hasBuff(spells.rapture)' , {
        ...
        {spells.powerWordShield, 'target.isAttackable and targettarget.isHealable and not targettarget.hasBuff(spells.powerWordShield) and not targettarget.hasDebuff(spells.weakenedSoul)' , "targettarget" , "powerWordShield_targettarget" },
        ...
        {spells.powerWordShield, 'mouseover.isHealable and not mouseover.hasBuff(spells.powerWordShield) and mouseover.hp < 0.995' , "mouseover" },
    }},
    ...
    {spells.rapture, 'spells.powerWordRadiance.charges == 0 and spells.powerWordRadiance.cooldown > 7 and heal.hasNotBuffAtonementCount(0.65) > 2' },
    ...
    {spells.schism, 'not player.isMoving and player.hasTalent(1,3) and spells.rapture.lastCasted(9)' , env.damageTarget , "schism_rapture" },
    {spells.schism, 'not player.isMoving and player.hasTalent(1,3) and spells.powerWordRadiance.lastCasted(7) and spells.powerWordRadiance.charges == 0' , env.damageTarget , "schism_charges" },

    {spells.mindbender, 'player.hasTalent(3,2) and spells.powerWordRadiance.lastCasted(6) and heal.hasBuffAtonementCount(0.82) > 2' , env.damageTarget },
    {spells.shadowfiend, 'not player.hasTalent(3,2) and spells.powerWordRadiance.lastCasted(6) and heal.hasBuffAtonementCount(0.82) > 2' , env.damageTarget },  
    ...
    {spells.powerWordShield, 'player.hp < 0.92 and player.myBuffDuration(spells.atonement) < 2 and not player.hasDebuff(spells.weakenedSoul)' , "player" , "powerWordShield_player" },
    {spells.powerWordShield, 'target.isAttackable and targettarget.isHealable and not targettarget.hasBuff(spells.powerWordShield) and targettarget.hp < 0.92 and not targettarget.hasDebuff(spells.weakenedSoul)' , "targettarget" , "powerWordShield_targettarget" },
    {spells.powerWordRadiance, 'not player.isMoving and heal.hasNotBuffAtonementCount(0.82) > 2 and mouseover.isHealable and mouseover.myBuffDuration(spells.atonement) < 2 and not mouseover.hasBuff(spells.atonement) and not spells.powerWordRadiance.isRecastAt("mouseover")' , "mouseover" , "radiance_mouseover_count"},
    {spells.powerWordShield, 'mouseover.isHealable and mouseover.myBuffDuration(spells.atonement) < 2 and mouseover.hp < 0.92 and not mouseover.hasDebuff(spells.weakenedSoul) and not mouseover.hasBuff(spells.atonement)' , "mouseover" , "powerWordShield_mouseover" },
    {spells.powerWordShield, 'mouseover.isHealable and mouseover.myBuffDuration(spells.atonement) < 2 and mouseover.isRaidTank and not mouseover.hasDebuff(spells.weakenedSoul) and not mouseover.hasBuff(spells.atonement)' , "mouseover" , "powerWordShield_mouseover" },

    {spells.shadowWordPain, 'target.isAttackable and target.myDebuffDuration(spells.shadowWordPain) < 4.8 and not spells.shadowWordPain.isRecastAt("target") and heal.lowestInRaid.hp > 0.82' , "target", "pain_target" },
    {spells.powerWordSolace, 'player.hasTalent(3,3)' , env.damageTarget },

    {spells.schism, 'not player.isMoving and player.hasTalent(1,3) and heal.hasBuffAtonementCount(0.82) >= 2' , env.damageTarget , "schism_count" },
    {spells.schism, 'not player.isMoving and player.hasTalent(1,3) and heal.hasBuffAtonement.hp < 0.40' , env.damageTarget , "schism_low" },
    ...
    {spells.holyNova, 'player.isMoving and heal.countLossInDistance(0.90,10) >= 3' , "target" },
    ...
    {{"nested"}, 'kps.mouseOver and mouseover.isAttackable' , {
        ...
        {spells.penance, 'heal.hasBuffAtonement.hp < 0.92', "mouseover" , "penance_mouseover"},
    ...
    {{"nested"}, 'kps.mouseOver and mouseover.isHealable' , {
        ...
        {spells.penance, 'heal.hasBuffAtonement.hp < 0.92 and mouseovertarget.isAttackable' , "mouseovertarget" , "penance_mouseovertarget" },
    ...
    {spells.smite, 'kps.mouseOver and not player.isMoving' , env.damageTarget , "smite_offensive" },
    {spells.shadowMend, 'not player.isMoving and heal.lowestInRaid.hp < 0.65', kps.heal.lowestInRaid ,  "shadowMend_lowest" },
    {spells.powerWordShield, 'heal.lowestInRaid.hp < 0.65 and not heal.lowestInRaid.hasDebuff(spells.weakenedSoul)', kps.heal.lowestInRaid ,  "powerWordShield_lowest" },
