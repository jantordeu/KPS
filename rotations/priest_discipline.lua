...
kps.rotations.register("PRIEST","DISCIPLINE",{
    ...
    {{"nested"}, 'player.hasBuff(spells.rapture)' , {
        ...
        {spells.penance, 'heal.lowestInRaid.myBuffDuration(spells.atonement) > 2 and heal.lowestInRaid.hp < 0.35' , kps.heal.lowestInRaid , "penance_defensive_lowest" },
        {spells.powerWordShield, 'mouseover.isHealable and not mouseover.hasBuff(spells.powerWordShield)' , "mouseover" }, 
        {spells.penance, 'heal.lowestTankInRaid.myBuffDuration(spells.atonement) > 2 and not heal.lowestTankInRaid.isUnit("player") and heal.lowestTankInRaid.hp < 0.35' , kps.heal.lowestTankInRaid  , "penance_defensive_tank" },
    ...
    {spells.schism, 'not player.isMoving and spells.rapture.cooldown > 0 and heal.countLossAtonementInRange(0.85) > 3' , env.damageTarget },
    {spells.schism, 'not player.isMoving and spells.powerWordRadiance.charges == 0 and heal.countLossAtonementInRange(0.85) > 3' , env.damageTarget },
    ...
    {spells.shadowfiend, 'not player.hasTalent(3,2) and spells.powerWordRadiance.charges == 0 and heal.countLossAtonementInRange(0.85) > 3' , env.damageTarget },
    {spells.penance, 'spells.rapture.cooldown > 0 and heal.countLossAtonementInRange(0.85) > 2' , env.damageTarget },
    {spells.penance, 'spells.powerWordRadiance.charges == 0 and heal.countLossAtonementInRange(0.85) > 2' , env.damageTarget  },
    {spells.powerWordSolace, 'spells.powerWordRadiance.charges == 0 and heal.countLossAtonementInRange(0.85) > 2' , env.damageTarget  },
    ...
    --PLAYER
    {spells.shadowMend, 'not player.isMoving and not player.hasMyBuff(spells.atonement) and player.hp < 0.95 and not spells.shadowMend.isRecastAt("player") and not spells.powerWordRadiance.isRecastAt("player")' , "player" , env.checkAtonement },
    {spells.powerWordShield, 'not player.hasMyBuff(spells.atonement) and not player.hasDebuff(spells.weakenedSoul) and not spells.shadowMend.isRecastAt("player") and not spells.powerWordRadiance.isRecastAt("player")' , "player" , env.checkAtonement },
    ...
    -- RADIANCE
    {spells.powerWordRadiance, 'not player.isMoving and kps.defensive and not heal.hasNotBuffAtonement.isUnit("player") and not spells.shadowMend.isRecastAt(heal.hasNotBuffAtonement.unit) and not spells.powerWordRadiance.isRecastAt(heal.hasNotBuffAtonement.unit)' , kps.heal.hasNotBuffAtonement , "radiance_defensive" },
    ...
    {spells.powerWordRadiance, 'not player.isMoving and heal.countLossInRange(0.85) > 3 and heal.lowestInRaid.hp < 0.85 and not spells.shadowMend.isRecastAt(heal.lowestInRaid.unit) and not spells.powerWordRadiance.isRecastAt(heal.lowestInRaid.unit)' , kps.heal.lowestInRaid , "radiance_lowest" },
    {spells.powerWordRadiance, 'not player.isMoving and not spells.powerWordRadiance.lastCasted(12) and not heal.hasNotBuffAtonement.isUnit("player") and heal.hasNotBuffAtonement.hp < 0.85 and not spells.shadowMend.isRecastAt(heal.hasNotBuffAtonement.unit) and not spells.powerWordRadiance.isRecastAt(heal.hasNotBuffAtonement.unit)' , kps.heal.hasNotBuffAtonement , "radiance_lowest" },
    ...
    {spells.azerite.refreshment, 'player.hp < 0.80' , "player" },
    ...
    --{spells.azerite.overchargeMana , 'spells.powerWordRadiance.charges > 0 and spells.schism.cooldown < 9 and heal.countLossInRange(0.85)*2 > heal.countInRange' },

    -- RACIAL
    --{spells.rocketBarrage, 'true' , env.damageTarget  },
    ...
    {spells.powerWordShield, 'not mouseover.hasDebuff(spells.weakenedSoul) and mouseover.isHealable and mouseover.hp < 0.85 and not mouseover.hasMyBuff(spells.atonement) and not spells.shadowMend.isRecastAt("mouseover") and not spells.powerWordRadiance.isRecastAt("mouseover")' , "mouseover" , "shield_mouseover" },  
    ...
    {spells.schism, 'not player.isMoving and heal.lowestTankInRaid.hp < 0.65 and heal.lowestTankInRaid.hasMyBuff(spells.atonement)' , env.damageTarget },
    {spells.schism, 'not player.isMoving and player.hp < 0.65 and player.hasMyBuff(spells.atonement)' , env.damageTarget },
    {spells.schism, 'kps.multiTarget and not player.isMoving' , env.damageTarget },
    {spells.penance, 'heal.countLossAtonementInRange(0.85) > 0' , env.damageTarget  },
    {spells.penance, 'kps.multiTarget' , env.damageTarget  },
    {spells.powerWordSolace, 'true' , env.damageTarget  },
    {spells.mindbender, 'kps.multiTarget and player.hasTalent(3,2) and target.hp > 0.30' , env.damageTarget },
    {spells.shadowfiend, 'kps.multiTarget and not player.hasTalent(3,2) and target.hp > 0.30' , env.damageTarget },
    {spells.holyNova, 'kps.holyNova and target.distance <= 10' },
    {spells.holyNova, 'player.hasBuff(spells.suddenRevelation) and target.distance <= 10' },
    {spells.smite, 'not player.isMoving and heal.lowestInRaid.hasMyBuff(spells.atonement) and heal.lowestInRaid.hp > 0.85' , env.damageTarget },
    ...
    --{spells.shadowCovenant, 'player.hasTalent(5,3) and heal.lowestInRaid.hp < 0.85' , kps.heal.lowestInRaid },
    ...
    -- LOWEST
    {spells.shadowMend, 'not player.isMoving and heal.lowestInRaid.hp < 0.85 and not heal.lowestInRaid.hasMyBuff(spells.atonement) and not spells.shadowMend.isRecastAt(heal.lowestInRaid.unit)' , kps.heal.lowestInRaid , "shadowMend_lowest" },
    {spells.powerWordShield, 'heal.lowestInRaid.hp < 0.85 and not heal.lowestInRaid.hasMyBuff(spells.atonement) and not spells.shadowMend.isRecastAt(heal.lowestInRaid.unit) and not heal.lowestInRaid.hasDebuff(spells.weakenedSoul)' , kps.heal.lowestInRaid , "shield_lowest" },
    -- SAFE
    {spells.smite, 'not player.isMoving and heal.lowestInRaid.hasMyBuff(spells.atonement) and heal.lowestInRaid.hp > 0.55' , env.damageTarget },
    -- EMERGENCY
    ...
    {spells.powerWordShield, 'player.hp < 0.55 and not player.hasDebuff(spells.weakenedSoul)' , "mouseover" , "shield_player_lowhp" },
    {spells.powerWordShield, 'heal.lowestInRaid.hp < 0.55 and heal.lowestInRaid.hp < heal.lowestTankInRaid.hp and not heal.lowestInRaid.hasDebuff(spells.weakenedSoul)' , kps.heal.lowestInRaid , "shield_lowestInRaid_lowhp" },
    {spells.powerWordShield, 'heal.lowestTankInRaid.hp < 0.55 and not heal.lowestTankInRaid.hasDebuff(spells.weakenedSoul)' , kps.heal.lowestTankInRaid , "shield_tank_lowhp" },
    -- TARGETTARGET
