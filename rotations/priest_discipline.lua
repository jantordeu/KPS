...
kps.rotations.register("PRIEST","DISCIPLINE",{
    ...
    {{"macro"}, 'player.useTrinket(0) and heal.lowestInRaid.hp < 0.80' , "/use 13" },
    ...
    {spells.shadowMend, 'not player.isMoving and not heal.lowestTankInRaid.isUnit("player") and not heal.lowestTankInRaid.hasMyBuff(spells.atonement) and not spells.shadowMend.isRecastAt(heal.lowestTankInRaid.unit)' , kps.heal.lowestTankInRaid , "shadowMend_tank" },
    {spells.powerWordShield, 'not heal.lowestTankInRaid.isUnit("player") not heal.lowestTankInRaid.hasMyBuff(spells.atonement) and not heal.lowestTankInRaid.hasDebuff(spells.weakenedSoul)' , kps.heal.lowestTankInRaid , "powerWordShield_tank" },
    -- PLAYER
    {spells.penance, 'player.myBuffDuration(spells.atonement) > 2 and player.hp < 0.40' , "player"  , "penance_defensive_player" },
    {spells.shadowMend, 'not player.isMoving and player.hp <  0.90 and not player.hasMyBuff(spells.atonement)' , "player" , "shadowMend_player" },
    ...
    {{"nested"}, 'kps.multiTarget' , {
        ...
        {spells.powerWordRadiance, 'not player.isMoving and heal.hasBuffAtonementCount(0.85) >= 3 and not heal.lowestTankInRaid.hasMyBuff(spells.atonement) and not heal.lowestTankInRaid.isUnit("player") and not spells.powerWordRadiance.isRecastAt(heal.lowestTankInRaid.unit)' , kps.heal.lowestTankInRaid , "radiance_tank" },
        {spells.powerWordRadiance, 'not player.isMoving and heal.hasBuffAtonementCount(0.85) >= 3 and not player.hasMyBuff(spells.atonement) and not spells.powerWordRadiance.isRecastAt("player")' , "player" , "radiance_player" },
        {spells.powerWordRadiance, 'not player.isMoving and heal.hasBuffAtonementCount(0.85) >= 3 and not heal.hasNotBuffAtonement.isUnit("player") and not spells.powerWordRadiance.isRecastAt("heal.hasNotBuffAtonement.unit")' , kps.heal.hasNotBuffAtonement , "radiance_hasNotBuffAtonement" },
        {spells.evangelism, 'spells.powerWordRadiance.lastCasted(7) and heal.countLossInRange(0.80) > 3' },
    }},

    -- NOT ISINGROUP
    {{"nested"}, 'kps.defensive' , {
        {spells.overchargeMana, 'not player.isInGroup and spells.schism.cooldown < 9' },
        {spells.rapture, 'player.isMoving and player.hp < 0.40' , "player" },
        ...
        {spells.powerWordShield, 'not player.hasMyBuff(spells.atonement) and not player.hasDebuff(spells.weakenedSoul)' , "player" },
        {spells.powerWordShield, 'mouseover.isFriend and not mouseover.hasMyBuff(spells.atonement) and mouseover.hp < 0.90 and not mouseover.hasDebuff(spells.weakenedSoul)' , "mouseover" },
        {spells.powerWordShield, 'targettarget.isFriend and targettarget.hp < 0.90 and not targettarget.hasMyBuff(spells.atonement) and not targettarget.hasDebuff(spells.weakenedSoul)' , "targettarget" , "powerWordShield_targettarget" },
        ...
        {spells.smite, 'not player.isMoving and player.hasBuff(spells.overchargeMana) and spells.overchargeMana.cooldown > 23' , env.damageTarget , "overchargeMana >>>" },
        {spells.schism, 'not player.isMoving and not player.isInGroup and player.hasBuff(spells.overchargeMana) and spells.overchargeMana.cooldown < 23' , env.damageTarget , "<<< overchargeMana" },
        {spells.schism, 'not player.isMoving and not player.isInGroup and not player.hasBuff(spells.overchargeMana) and spells.powerWordSolace.cooldown < 7' , env.damageTarget },
        {spells.schism, 'not player.isMoving and not player.isInGroup and not player.hasBuff(spells.overchargeMana) and spells.penance.cooldown < 7' , env.damageTarget },
        {spells.powerWordSolace, 'true' , env.damageTarget },
        {spells.penance, 'player.hasBuff(spells.overchargeMana) and spells.overchargeMana.cooldown < 23' , env.damageTarget , "<<< overchargeMana" },
        {spells.penance, 'not player.hasBuff(spells.overchargeMana) and spells.overchargeMana.cooldown > 2' , env.damageTarget },
        {spells.mindbender, 'player.hasTalent(3,2) and player.hasBuff(spells.overchargeMana)' , env.damageTarget },
        {spells.shadowfiend, 'not player.hasTalent(3,2) and player.hasBuff(spells.overchargeMana)' , env.damageTarget },
    ...
    {{"nested"}, 'player.hasBuff(spells.overchargeMana) and spells.overchargeMana.cooldown < 23' , {
        ...
        {spells.schism, 'not player.isMoving and heal.hasBuffAtonementCount(0.80) >= 3' , env.damageTarget , "overchargeMana" },
        {spells.schism, 'not player.isMoving and heal.hasBuffAtonementCount(0.65) >= 1' , env.damageTarget , "overchargeMana" },
        ...
        {spells.penance, 'heal.hasBuffAtonementCount(0.85) >= 1' , env.damageTarget , "overchargeMana" },
    ...
    {{"nested"}, 'player.hasBuff(spells.overchargeMana) and spells.overchargeMana.cooldown > 23' , {
        ...
        {spells.evangelism, 'spells.powerWordRadiance.lastCasted(7)' },
        {spells.powerWordShield, 'heal.hasNotBuffAtonementCount(0.80) >= 1 and not heal.hasNotBuffAtonement.hasDebuff(spells.weakenedSoul) and not heal.hasNotBuffAtonement.isUnit("player")' , kps.heal.hasNotBuffAtonement , "powerWordShield_hasNotBuffAtonement_multiTarget" },
    ...
    {spells.rapture, 'heal.hasNotBuffAtonementCount(0.85) >= 3 and spells.powerWordRadiance.charges == 0 and spells.powerWordRadiance.cooldown > 4 and spells.penance.cooldown > 4 and spells.schism.cooldown > 4' },
    ...
    {spells.shadowCovenant, 'player.hasTalent(5,3) and heal.countLossInRange(0.85) >= 3' , kps.heal.lowestInRaid },
    ...
    {spells.schism, 'not player.isMoving and heal.hasBuffAtonementCount(0.65) >= 1 and spells.powerWordRadiance.lastCasted(7)' , env.damageTarget , "hasBuffAtonementCount" },
    {spells.schism, 'not player.isMoving and heal.hasBuffAtonementCount(0.80) >= 3' , env.damageTarget , "hasBuffAtonementCount" },
    {spells.penance, 'heal.hasBuffAtonementCount(0.85) >= 1' , env.damageTarget , "hasBuffAtonementCount" },
    ...
    {spells.shadowWordPain,'mouseover.isAttackable and mouseover.inCombat and player.hasTalent(6,1) and mouseover.myDebuffDuration(spells.purgeTheWicked) < 4.8 and not spells.shadowWordPain.isRecastAt("mouseover")', "mouseover" , "pain_mouseover"},    
    ...
    {spells.shadowMend, 'not player.isMoving and mouseover.hp < 0.80 and not mouseover.hasMyBuff(spells.atonement) and not spells.shadowMend.isRecastAt("mouseover")' , "mouseover" , "shadowMend_mouseover"},
    {spells.powerWordShield, 'not mouseover.hasMyBuff(spells.atonement) and mouseover.hp < 0.85 and not mouseover.hasDebuff(spells.weakenedSoul)' , "mouseover" , "powerWordShield_mouseover" },
    {spells.schism, 'mouseovertarget.isAttackable and mouseover.hp < 0.65' , "mouseovertarget" , "schism_mouseovertarget" },
    {spells.powerWordSolace, 'mouseovertarget.isAttackable' , "mouseovertarget" , "solace_mouseovertarget" },
    {spells.penance, 'mouseovertarget.isAttackable and mouseover.hp < 0.90' , "mouseovertarget" , "penance_mouseovertarget" },
    ...
    {spells.shadowWordPain,'player.hasTalent(6,1) and mouseover.myDebuffDuration(spells.purgeTheWicked) < 4.8 and not spells.shadowWordPain.isRecastAt("mouseover")', "mouseover" , "pain_mouseover"},    
    {spells.shadowMend, 'not player.isMoving and mouseovertarget.isHealable and mouseovertarget.hp < 0.80 and not mouseovertarget.hasMyBuff(spells.atonement) and not spells.shadowMend.isRecastAt("mouseovertarget")' , "mouseovertarget" , "shadowMend_mouseovertarget"},
    {spells.powerWordShield, 'mouseovertarget.isHealable and mouseovertarget.hp < 0.85 and not mouseovertarget.hasMyBuff(spells.atonement) and not mouseovertarget.hasDebuff(spells.weakenedSoul)' , "mouseovertarget" , "powerWordShield_mouseovertarget" },
    {spells.schism, 'mouseovertarget.isHealable and mouseovertarget.hp < 0.65', "mouseover" , "schism_mouseover"},
    {spells.powerWordSolace, 'mouseovertarget.isHealable', "mouseover" , "solace_mouseover"},
    {spells.penance, 'mouseovertarget.isHealable and mouseovertarget.hp < 0.90', "mouseover" , "penance_offensive_mouseover"},
    ...
    {spells.smite, 'not player.isMoving and mouseovertarget.isHealable', "mouseover" , "smite_mouseover"},
    ...
    {spells.smite, 'not player.isMoving and heal.hasNotBuffAtonementCount(0.65) == 0' , env.damageTarget , "hasNotBuffAtonementCount" },
    ...
    {spells.powerWordShield, 'heal.hasNotBuffAtonementCount(0.65) >= 1 and not heal.hasNotBuffAtonement.hasDebuff(spells.weakenedSoul) and not heal.hasNotBuffAtonement.isUnit("player")' , kps.heal.hasNotBuffAtonement , "powerWordShield_hasNotBuffAtonement" },
    -- EMERGENCY
