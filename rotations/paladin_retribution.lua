...
kps.rotations.register("PALADIN","RETRIBUTION",
{
    ...
    -- AZERITE
    {spells.concentratedFlame, 'target.isAttackable and target.distanceMax <= 30' , "target" },
    ...
    {spells.theUnboundForce, 'target.isAttackable and target.distanceMax <= 30' , "target" },
   
    {spells.inquisition, 'player.hasTalent(7,3) and player.holyPower >= 2 and player.myBuffDuration(spells.inquisition) < 5' , "target" , "inquisition" },
    {spells.inquisition, 'player.hasTalent(7,3) and player.holyPower >= 1 and player.myBuffDuration(spells.inquisition) < 12' , "target" , "inquisition" },
    {{"nested"},'kps.cooldowns and player.holyPower >= 3', {
        {spells.avengingWrath, 'target.isAttackable and player.hasTalent(7,3) and player.myBuffDuration(spells.inquisition) > 20 and target.distanceMax <= 10' },
    ...
    {spells.wakeOfAshes, 'player.holyPower == 0 and target.distanceMax <= 10' , "target" },   
    {spells.bladeOfJustice, 'player.holyPower <= 3 and target.distanceMax <= 10' , "target" },   -- Generates 2 Holy Power. 10 sec cd
    {spells.wakeOfAshes, 'player.holyPower <= 1 and target.distanceMax <= 10' , "target" },
