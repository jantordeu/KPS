...
kps.rotations.register("PRIEST","HOLY",{
    ...
    {{"macro"}, 'spells.flashHeal.shouldInterrupt(0.85, kps.defensive and not player.hasBuff(spells.flashConcentration))' , "/stopcasting" },
    ...
    {{"nested"}, 'player.hasBuff(spells.spiritOfRedemption) and not heal.lowestInRaid.isUnit("player")' ,{
        ...
        {spells.heal, 'player.hasBuff(spells.flashConcentration) and player.buffStacks(spells.flashConcentration) == 5' , kps.heal.lowestInRaid},
    ...
    {{"nested"},'player.hasBuff(spells.surgeOfLight)', {
		{spells.flashHeal, 'IsEquippedItem(173249) and not player.hasBuff(spells.flashConcentration) and not spells.flashHeal.lastCasted(3)' , kps.heal.lowestInRaid , "flashHeal_Concentration_buff"  },
    	{spells.flashHeal, 'player.buffStacks(spells.surgeOfLight) == 2 and player.hasBuff(spells.flashConcentration)' , kps.heal.lowestInRaid , "flashHeal_Concentration_stacks"  },
    	{spells.flashHeal, 'player.hasBuff(spells.flashConcentration) and player.buffDuration(spells.flashConcentration) < 3 and not spells.flashHeal.isRecastAt(heal.lowestInRaid.unit) and not spells.flashHeal.lastCasted(3)' , kps.heal.lowestInRaid , "flashHeal_Concentration_duration" },    
    }},
	{spells.flashHeal, 'not player.isMoving and not player.hasBuff(spells.surgeOfLight) and player.hasBuff(spells.flashConcentration) and player.buffDuration(spells.flashConcentration) < 5 and not spells.flashHeal.isRecastAt(heal.lowestInRaid.unit) and not spells.flashHeal.lastCasted(5)' , kps.heal.lowestInRaid , "flashHeal_Concentration_duration" },    
    ...
    {{"nested"}, 'player.hp < 0.80 and player.isPVP' ,{
        ...
        {spells.heal, 'not player.isMoving and player.hp < 0.65 and player.hasBuff(spells.flashConcentration) and player.buffStacks(spells.flashConcentration) > 3 and not spells.heal.isRecastAt("player")' , "player" , "heal_player_Concentration" },
        {spells.flashHeal, 'player.hp < 0.55 and not spells.heal.isRecastAt("player")' , "player" , "flashHeal_player" },
    ...
    {{"nested"}, 'not player.isMoving and player.hasBuff(spells.apotheosis)',{
        {spells.heal, 'player.hasBuff(spells.flashConcentration) and player.buffStacks(spells.flashConcentration) > 3 and heal.lowestInRaid.hpIncoming  < 0.80 and heal.lowestInRaid.hpIncoming  < heal.lowestTankInRaid.hpIncoming ' , kps.heal.lowestInRaid , "heal_lowest_apotheosis" },
    	{spells.heal, 'player.hasBuff(spells.flashConcentration) and player.buffStacks(spells.flashConcentration) > 3 and heal.lowestTankInRaid.hpIncoming  < 0.80' , kps.heal.lowestTankInRaid , "heal_tank_apotheosis"  },
        {spells.flashHeal, 'heal.lowestTankInRaid.hp < 0.65' , kps.heal.lowestTankInRaid , "flashHeal_tank_apotheosis" },
        {spells.flashHeal, 'heal.lowestInRaid.hp < 0.65' , kps.heal.lowestInRaid , "flashHeal_lowest_apotheosis" },
    	{spells.prayerOfHealing, 'heal.countLossInRange(0.80) > 3' , kps.heal.lowestTankInRaid , "POH_apotheosis" },
    	{spells.prayerOfHealing, 'heal.countLossInRange(0.80) > 2 and not player.isInRaid' , kps.heal.lowestInRaid , "POH_apotheosis" },
    ...
    {spells.prayerOfHealing, 'not player.isMoving and mouseover.isHealable and mouseover.hp < 0.80 and heal.countLossInRange(0.80) > 2 and not player.isInRaid' , "mouseover" , "prayerOfHealing_mouseover" },
    {spells.prayerOfHealing, 'not player.isMoving and mouseover.isHealable and mouseover.hp < 0.80 and heal.countLossInRange(0.80) > 3' , "mouseover" , "prayerOfHealing_mouseover" },

    -- URGENCE
    {{"nested"}, 'not player.isMoving and heal.lowestInRaid.hp < 0.65 ' ,{
        {spells.heal, 'heal.lowestInRaid.hp < heal.lowestTankInRaid.hp and not spells.heal.isRecastAt(heal.lowestInRaid.unit) and player.hasBuff(spells.flashConcentration) and player.buffStacks(spells.flashConcentration) > 3' , kps.heal.lowestInRaid ,"heal_lowest_Concentration" },
        {spells.heal, 'not spells.heal.isRecastAt(heal.lowestTankInRaid.unit) and player.hasBuff(spells.flashConcentration) and player.buffStacks(spells.flashConcentration) > 3' , kps.heal.lowestTankInRaid ,"heal_tank_Concentration" },
        {spells.flashHeal, 'heal.lowestInRaid.hp < 0.55 and heal.lowestInRaid.hp < heal.lowestTankInRaid.hp and not spells.flashHeal.isRecastAt(heal.lowestInRaid.unit)' , kps.heal.lowestInRaid ,"flashHeal_lowest" },
        {spells.flashHeal, 'heal.lowestInRaid.hp < 0.55 and not spells.flashHeal.isRecastAt(heal.lowestTankInRaid.unit)' , kps.heal.lowestTankInRaid ,"flashHeal_tank" },
    }},
    ...
    {{"nested"}, 'not player.isMoving and spells.prayerOfHealing.lastCasted(4) and player.hasBuff(spells.flashConcentration) and player.buffStacks(spells.flashConcentration) > 3' ,{
    	{spells.heal, 'player.hp < 0.55' , "player" , "heal_POH_Concentration" },
    ...
    {{"nested"}, 'not player.isMoving and spells.prayerOfMending.lastCasted(4)' ,{
        ...
        {spells.prayerOfHealing, 'not player.isMoving and heal.countLossInRange(0.80) > 3 and heal.lowestInRaid.hp < heal.lowestTankInRaid.hp' , kps.heal.lowestInRaid , "POH_POM" },
        {spells.prayerOfHealing, 'not player.isMoving and heal.countLossInRange(0.80) > 3' , kps.heal.lowestTankInRaid , "POH_POM" },
        {spells.prayerOfHealing, 'not player.isMoving and heal.countLossInRange(0.80) > 2 and not player.isInRaid' , kps.heal.lowestInRaid , "POH_POM" },
    }},
    -- "Prayer of Healing"
    ...
    {{"nested"}, 'player.isMoving' ,{
        ...
        {spells.renew, 'heal.lowestInRaid.hpIncoming < 0.85 and not heal.lowestInRaid.hasBuff(spells.renew) and not player.isInRaid' , kps.heal.lowestInRaid },
    }},
    ...
    -- DAMAGE
    {spells.shadowWordDeath, 'target.isAttackable and target.hp < 0.20 and player.hp > 0.55' , "target" , "shadowWordDeath" },
    {spells.shadowWordDeath, 'mouseover.inCombat and mouseover.isAttackable and mouseover.hp < 0.20 and player.hp > 0.55' , "mouseover" , "shadowWordDeath" },
    ...
    -- "Soins rapides" PARTY
    {spells.flashHeal, 'not player.isMoving and heal.lowestInRaid.hpIncoming < 0.55 and heal.lowestInRaid.hpIncoming < heal.lowestTankInRaid.hpIncoming and not player.isInRaid' , kps.heal.lowestInRaid , "flashHeal_lowest" },
    {spells.flashHeal, 'not player.isMoving and heal.lowestTankInRaid.hpIncoming < 0.55 and not player.isInRaid' , kps.heal.lowestTankInRaid , "flashHeal_tank"  },
    -- "Soins"
