...
kps.rotations.register("PRIEST","HOLY",{
    ...
    {{"nested"},'player.hasBuff(spells.surgeOfLight)', {
        ...
        {spells.flashHeal, 'player.buffStacks(spells.surgeOfLight) == 2 and heal.lowestInRaid.hp < 0.85' , kps.heal.lowestInRaid  },
    }},
	...
	--{spells.flashHeal, 'not player.isMoving and IsEquippedItem(173249) and not player.hasBuff(spells.flashConcentration) and not spells.flashHeal.isRecastAt(heal.lowestInRaid.unit) and not spells.flashHeal.lastCasted(5)' , kps.heal.lowestInRaid , "flashHeal_Concentration_duration" },    
    ...
    -- MOUSEOVER
    {spells.heal, 'not player.isMoving and mouseover.isHealable and mouseover.hpIncoming < 0.80 and player.hasBuff(spells.flashConcentration) and player.buffStacks(spells.flashConcentration) > 3' , "mouseover" , "heal_mouseover" },
    ...
    -- URGENCE
    {{"nested"}, 'not player.isMoving and heal.lowestInRaid.hpIncoming < 0.80 ' ,{
        {spells.heal, 'heal.lowestInRaid.hp < heal.lowestTankInRaid.hp and not spells.heal.isRecastAt(heal.lowestInRaid.unit) and player.hasBuff(spells.flashConcentration) and player.buffStacks(spells.flashConcentration) > 3' , kps.heal.lowestInRaid ,"heal_lowest_Concentration_urg" },
        {spells.heal, 'heal.lowestTankInRaid.hpIncoming < 0.80 and not spells.heal.isRecastAt(heal.lowestTankInRaid.unit) and player.hasBuff(spells.flashConcentration) and player.buffStacks(spells.flashConcentration) > 3' , kps.heal.lowestTankInRaid ,"heal_tank_Concentration_urg" },
        {spells.flashHeal, 'heal.lowestInRaid.hp < 0.55 and heal.lowestInRaid.hp < heal.lowestTankInRaid.hp and not spells.flashHeal.isRecastAt(heal.lowestInRaid.unit)' , kps.heal.lowestInRaid ,"flashHeal_lowest_urg" },
        {spells.flashHeal, 'heal.lowestInRaid.hp < 0.55 and not spells.flashHeal.isRecastAt(heal.lowestTankInRaid.unit)' , kps.heal.lowestTankInRaid ,"flashHeal_tank_urg" },
    }},
    ...
    {{"nested"}, 'not player.isMoving and spells.prayerOfMending.lastCasted(4) and player.hasBuff(spells.flashConcentration) and player.buffStacks(spells.flashConcentration) > 3' ,{
        {spells.heal, 'player.hp < 0.55' , "player" , "heal_POM_Concentration" },
