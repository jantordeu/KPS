...
kps.rotations.register("PRIEST","HOLY",{
    ...
    {{"nested"},'player.hasBuff(spells.surgeOfLight)', {
		{spells.flashHeal, 'IsEquippedItem(173249) and not player.hasBuff(spells.flashConcentration) and not spells.flashHeal.lastCasted(5)' , kps.heal.lowestInRaid , "flashHeal_Concentration_buff"  },
    	...
    	{spells.flashHeal, 'player.hasBuff(spells.flashConcentration) and player.buffDuration(spells.flashConcentration) < 5 and not spells.flashHeal.isRecastAt(heal.lowestInRaid.unit) and not spells.flashHeal.lastCasted(5)' , kps.heal.lowestInRaid , "flashHeal_Concentration_duration" },    
    }},
    ...
    {{"nested"}, 'player.hp < 0.80 and player.isPVP' ,{
        ...
        {spells.heal, 'not player.isMoving and player.hasBuff(spells.flashConcentration) and player.buffStacks(spells.flashConcentration) == 5' },
    ...
    {{"nested"}, 'not player.isMoving and player.hasBuff(spells.apotheosis)',{
        ...
        {spells.heal, 'heal.lowestInRaid.hp < 0.80 and player.hasBuff(spells.flashConcentration)' , kps.heal.lowestInRaid , "heal_lowest_apotheosis" },
    }},
    ...
    -- URGENCE
    {{"nested"}, 'not player.isMoving and heal.lowestInRaid.hp < 0.80 and player.hasBuff(spells.flashConcentration) and player.buffStacks(spells.flashConcentration) > 3' ,{
        {spells.heal, 'player.hp < 0.40 and not spells.heal.isRecastAt("player")' , "player" ,"heal_player_Concentration" },
        {spells.heal, 'heal.lowestInRaid.hp < heal.lowestTankInRaid.hp and not spells.heal.isRecastAt(heal.lowestInRaid.unit)' , kps.heal.lowestInRaid ,"heal_lowest_Concentration" },
        {spells.heal, 'not spells.heal.isRecastAt(heal.lowestTankInRaid.unit)' , kps.heal.lowestTankInRaid ,"heal_tank_Concentration" },
    }},
    {{"nested"}, 'not player.isMoving and heal.lowestInRaid.hp < 0.65' ,{
        {spells.flashHeal, 'player.hp < 0.40 and not spells.heal.isRecastAt("player")' , "player" ,"heal_player_Concentration" },
        {spells.flashHeal, 'heal.lowestInRaid.hp < heal.lowestTankInRaid.hp and not spells.flashHeal.isRecastAt(heal.lowestInRaid.unit)' , kps.heal.lowestInRaid ,"flashHeal_lowest" },
        {spells.flashHeal, 'not spells.flashHeal.isRecastAt(heal.lowestTankInRaid.unit)' , kps.heal.lowestTankInRaid ,"flashHeal_tank" },
    }},
    ...
    {{"nested"}, 'not player.isMoving and spells.prayerOfHealing.lastCasted(4) and player.hasBuff(spells.flashConcentration) and player.buffStacks(spells.flashConcentration) > 3' ,{
    	{spells.heal, 'player.hp < 0.40' , "player" , "heal_POH_Concentration" },
    ...
    {{"nested"}, 'not player.isMoving and spells.prayerOfHealing.lastCasted(4)' ,{
        {spells.flashHeal, 'player.hp < 0.40' , "player" , "flashHeal_POH" },
        {spells.flashHeal, 'heal.lowestInRaid.hp < 0.65 and heal.lowestInRaid.hp < heal.lowestTankInRaid.hp' , kps.heal.lowestInRaid , "flashHeal_POH" },
        {spells.flashHeal, 'heal.lowestTankInRaid.hp < 0.65' , kps.heal.lowestTankInRaid , "flashHeal_POH" },
    }},
    ...
    {{"nested"}, 'not player.isMoving and spells.prayerOfMending.lastCasted(4)' ,{
        {spells.flashHeal, 'player.hp < 0.40' , "player" ,"flashHeal_POM" },
        {spells.flashHeal, 'heal.lowestInRaid.hp < 0.65 and heal.lowestInRaid.hp < heal.lowestTankInRaid.hp' , kps.heal.lowestInRaid ,"flashHeal_POM" },
        {spells.flashHeal, 'heal.lowestTankInRaid.hp < 0.65' , kps.heal.lowestTankInRaid ,"flashHeal_POM" },
    }},
    -- "Prayer of Mending" LASTCAST
    {{"nested"}, 'not player.isMoving and spells.prayerOfMending.lastCasted(4)' ,{
