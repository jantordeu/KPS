...
kps.rotations.register("PRIEST","HOLY",
{
    ...
    {{"macro"}, 'spells.prayerOfHealing.shouldInterrupt(heal.countLossInRange(0.80), kps.defensive)' , "/stopcasting" },
    ...
    {{"macro"}, 'not player.isInRaid and heal.countLossInDistance(0.80) > 1 and spells.holyWordSanctify.cooldown == 0' , "/cast [@player] "..HolyWordSanctify },
    -- "Door of Shadows" 
    ...
    {spells.mindgames, 'not player.isMoving and heal.lowestTankInRaid.hp < 0.80' , env.damageTarget },
    ...
    --{{"macro"}, 'player.useTrinket(1) and player.timeInCombat > 5' , "/use [@player] 14" },
    ...
    {{"nested"}, 'not player.isMoving and player.hasBuff(spells.apotheosis)',{
    	...
    	{spells.prayerOfHealing, 'heal.countLossInRange(0.70) > 2' , kps.heal.lowestInRaid , "POH_apotheosis" },
    ...
    {spells.powerInfusion, 'heal.lowestInRaid.hp < 0.55 and heal.countLossInRange(0.70) > 4' },
    {spells.powerInfusion, 'not player.isInRaid and heal.lowestInRaid.hp < 0.55 and heal.countLossInRange(0.70) > 2' },
    ...
    {spells.heal, 'kps.concentration and not player.isMoving and player.hp < 0.55 and player.hasBuff(spells.flashConcentration)', "player", "heal_player_Concentration"  },
    {{"nested"}, 'kps.concentration' ,{
        {spells.holyWordSerenity, 'focus.isFriend and focus.hp < 0.70' , "focus" },
        {spells.heal, 'focus.isFriend and not player.isMoving and focus.hp < 0.70 and player.hasBuff(spells.flashConcentration)' , "focus" },
        {spells.flashHeal, 'focus.isFriend and not player.isMoving and focus.hp < 0.55' , "focus" },
        {spells.renew, 'player.isMoving and focus.isFriend and not focus.hasBuff(spells.renew)' , "focus" },
        {spells.holyWordSerenity, 'heal.lowestTankInRaid.hp < 0.70' , kps.heal.lowestTankInRaid},
        {spells.heal, 'not player.isMoving and heal.lowestTankInRaid.hp < 0.70 and player.hasBuff(spells.flashConcentration)' , kps.heal.lowestTankInRaid  },
        {spells.flashHeal, 'not player.isMoving and heal.lowestTankInRaid.hp < 0.55' , kps.heal.lowestTankInRaid  },
        {spells.renew, 'player.isMoving and not heal.lowestTankInRaid.hasBuff(spells.renew)' , kps.heal.lowestTankInRaid },
    }},
    ...
    {spells.heal, 'not player.isMoving and spells.prayerOfHealing.lastCasted(5) and player.hasBuff(spells.flashConcentration) and heal.lowestInRaid.hp < 0.70' , kps.heal.lowestInRaid , "heal_POH_Concentration" },
    ...
    {spells.heal, 'not player.isMoving and heal.lowestInRaid.hp < 0.70 and player.hasBuff(spells.flashConcentration)', kps.heal.lowestInRaid, "heal_lowest_Concentration"  },
    ...
    -- "Soins"
    {spells.heal, 'not player.isMoving and heal.lowestInRaid.hp < 0.70 and heal.lowestInRaid.hp < heal.lowestTankInRaid.hp and heal.lowestInRaid.hp < player.hp', kps.heal.lowestInRaid, "heal_lowest"  },
    ...
    -- Damage
    {spells.holyFire, 'not player.isMoving and player.mana > 0.80' , env.damageTarget },
    {spells.smite, 'not player.isMoving', env.damageTarget },
